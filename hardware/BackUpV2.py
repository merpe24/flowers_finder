...
class App:
    def __init__(self):
        ...
        # This prevents the RFID library from resetting your LED pins later
        self.rfid = RFID()

        # 2. Setup GPIO Pins AFTER RFID is ready
        if HARDWARE_AVAILABLE:
            # Re-confirm mode in case RFID library changed it to BOARD
            GPIO.setmode(GPIO.BCM) 
            
            # Use a loop to ensure clean setup
            for pin in [GREEN_LED, RED_LED, BUZZER]:
                GPIO.setup(pin, GPIO.OUT, initial=GPIO.LOW)
            
            print("[Hardware] LEDs and Buzzer configured on BCM pins")
